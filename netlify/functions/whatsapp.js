...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (adminDoc?.activo) {
      ...
      if (!botDoc?.activo) return xml('', from); // bot off para admin
      // Modo prueba: simular como cliente
      if (botDoc?.modoPrueba) {
        ...
        return xml(r.texto || '...', from);
      }
      // Bot on para admin — respuesta directa sin guardar en CRM
      const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
      ...
      return xml(r.content[0]?.text?.trim() || '...', from);
    }
    ...
    if (!cliente) {
      ...
      if (ref?.name) {
        ...
        await notificarAdmins(`👤 ¡Nuevo contacto por WhatsApp!\n📱 +52${tel}\nEdítalo en la app.`);
      }
    }
    if (!cliente) return xml('Hola! Bienvenid@ a Barbería Zaira 💅 En breve te atendemos.', from);
    ...
    return xml(respuesta, from);

  } catch(err) {
    console.error('Error handler:', err);
    return xml('Ahorita no puedo responder. Intenta en un momento 🙏');
  }
};
